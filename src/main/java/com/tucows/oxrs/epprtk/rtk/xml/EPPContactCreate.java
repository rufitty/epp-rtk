...
/*
 * $Header: /cvsroot/epp-rtk/epp-rtk/java/src/com/tucows/oxrs/epprtk/rtk/xml/EPPContactCreate.java,v 1.1 2004/12/07 15:53:27 ewang2004 Exp $
 * $Revision: 1.1 $
 ...
 * @see org.openrtk.idl.epprtk.contact.epp_ContactCreate
...
public class EPPContactCreate extends EPPContactBase implements epp_ContactCreate
{
     ...
     * @param value org.openrtk.idl.epprtk.epp_ContactCreateReq
     */
     ...
     * @return value org.openrtk.idl.epprtk.epp_ContactCreateRsp
     */
    ...
    public String toXML() throws epp_XMLException
    {
        ...
        if ( action_request_.m_ascii_address != null )
        {
            // XXX the usage of m_ascii_address and m_i15d_address
            //     is deprecated, so this code exists for backward compatibility
            if ( action_request_.m_ascii_address != null ) { action_request_.m_ascii_address.m_type = epp_ContactPostalInfoType.INT; }
            Element ascii_address = addressToXML(doc, "contact:postalInfo", action_request_.m_ascii_address);
            if ( ascii_address != null )
            {
                contact_create.appendChild(ascii_address);
            }

            if ( action_request_.m_i15d_address != null ) { action_request_.m_i15d_address.m_type = epp_ContactPostalInfoType.LOC; }
            Element i15d_address = addressToXML(doc, "contact:postalInfo", action_request_.m_i15d_address);
            if ( i15d_address != null )
            {
                contact_create.appendChild(i15d_address);
            }
        }
        else if ( action_request_.m_addresses != null &&
                  action_request_.m_addresses.length > 0 )
        {
            ...
            for (Iterator it = addresses.iterator(); it.hasNext();)
            {
                ...
                if ( address != null )
                {
                    contact_create.appendChild(address);
                }
            }
        }
